...
A local build produces the installers and a `.sig` beside each one, but **not**
`latest.json` — that manifest is generated by `tauri-action` when it creates the
release, because it has to contain the download URLs, which only exist once the
release does. A local build is therefore good for checking that signing works,
and cannot be published by hand without writing the manifest yourself.
