...
export function Header({ onOpenPalette }: { onOpenPalette: () => void }) {
  ...
  return (
    <header className="flex h-11 shrink-0 items-center gap-2 border-b border-border bg-background/80 px-2 backdrop-blur">
      <button
        onClick={onOpenPalette}
        aria-label="Open command palette"
        className="group flex h-7 max-w-sm flex-1 items-center gap-2 rounded-md border border-border bg-secondary/40 px-2 text-xs text-muted-foreground transition-all duration-150 hover:border-primary/40 hover:bg-secondary hover:text-foreground focus-visible:border-primary/40"
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        ...
        <Button variant="ghost" size="icon" className="size-7" title="Toggle theme" aria-label="Toggle theme" onClick={toggleTheme}>
          {theme === "dark" ? <Sun /> : <Moon />}
        </Button>
        <Button variant="ghost" size="icon" className="size-7" title="Settings" aria-label="Settings" onClick={() => openView({ kind: "settings" })}>
          <Settings />
        </Button>
      </div>
    </header>
